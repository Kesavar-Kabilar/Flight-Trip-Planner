...
class FlightSegment:
    ...
    def __init__(self, fid: str, dep: datetime.datetime, arr: datetime.datetime,
        ...
        x = str(arr-dep).split(":")
        self._flight_duration = datetime.time(int(x[0]), int(x[1]), int(x[2]))
    ...
    def book_seat(self, cid: int, seat_type: str) -> None:
        ...
        if self.seat_availability[seat_type] > 0:
            for customer in self._manifest:
                if customer[0] == cid:
                    self.seat_availability[customer[1]] += 1
...
class Trip:
    ...
    def get_in_flight_time(self) -> int:
        ...
        """
        total_duration = 0
        for flight_segment in self._flights:
            total_duration += flight_segment.get_duration().hour * 60
            total_duration += flight_segment.get_duration().minute
    ...
    def get_total_trip_time(self) -> int:
        ...
        trip_time = self._flights[-1].get_times()[1]\
                    - self._flights[0].get_times()[0]

        trip_time_minutes = trip_time.days * 1440 + trip_time.seconds / 60

        return trip_time_minutes
